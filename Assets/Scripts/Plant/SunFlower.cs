...
public class SunFlower : PlantBase
{
    ...
    {
        InvokeRepeating("CreateSun", 3, 10);
    }
    ...
    {
        ChangeFlowerColor();
        Sun sun = GameObject.Instantiate(SunManager.Instance.GetSunPrefabByType(sunType), transform.position, Quaternion.identity, SunManager.Instance.transform).GetComponent<Sun>();
